...
import { type AdminCategoryInput, adminCategorySchema } from "@/lib/validations/admin-catalog";
...
export function CategoryManager({ items }: CategoryManagerProps) {
  ...
  const [editingItem, setEditingItem] = useState<AdminCategoryListItem | null>(null);
      ...
      if (!result.success) {
        if (result.fieldErrors) {
          for (const [key, messages] of Object.entries(result.fieldErrors)) {
            if (messages?.[0]) {
              setError(key as keyof AdminCategoryInput, { message: messages[0] });
            }
          }
        }
          ...
          {sortedItems.map((item) => (
            <article className="rounded-card border border-default bg-background p-4" key={item.id}>
              <div className="flex items-start justify-between gap-4">
                ...
                <StatusToggleButton id={item.id} isActive={item.isActive} table="categories" />
              </div>
    ...
    >
      <form className="space-y-4" onSubmit={onSubmit}>
        ...
        <Field label="Nome" error={errors.name?.message}>
          <input className={fieldClassName} placeholder="Rações" {...register("name")} />
        </Field>
        <Field label="Slug" error={errors.slug?.message}>
          <input className={fieldClassName} placeholder="racoes" {...register("slug")} />
        </Field>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Ícone" error={errors.icon?.message}>
            <input className={fieldClassName} placeholder="bowl" {...register("icon")} />
          </Field>
          <Field label="Cor" error={errors.color?.message}>
            <input className={fieldClassName} placeholder="#00A9C8" {...register("color")} />
          </Field>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Ordem" error={errors.sortOrder?.message}>
            <input className={fieldClassName} min="0" type="number" {...register("sortOrder")} />
          </Field>
...
function appendCategoryFormData(formData: FormData, values: AdminCategoryInput) {
  if (values.id) {
